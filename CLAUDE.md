...
| ทุก 30 นาที (รอบแรกวินาที 600) | `catchup_job`: เรียก `startup_*_job` ของรายงานรายวันซ้ำ (heartbeat / digest / confirm / reminder / openpos / calendar / scan) — เก็บรอบที่พลาดเพราะเครื่องหลับ และรอบที่ล้ม · ข้าม job ที่ `_done` วันนี้แล้วและไม่เคยล้ม (ไม่ build ซ้ำเปล่า ๆ) · ไม่รวมเก็บตกข่าว |
...
- คำสั่ง `สถานะ` ต่อท้ายบรรทัดข่าว SET ว่า `· ล่าสุด: <อาการ> @HH:MM` (เมื่อล้ม ≥1 รอบ) และ `· backoff ถึง HH:MM` (ระหว่างเว้นระยะ) — ค่ามาจาก `_news_last_error`/`_news_next_try` ฉีดผ่าน `build_status_report` เหมือนค่า memory ตัวอื่น
