...
telegram_bot.py   ← entry point: handler + scheduled jobs ทั้งหมด
  ...
  ├── bot_status.py   ← คำสั่ง "สถานะ" / /status: สุขภาพบอทจากไฟล์+memory ไม่ยิง API (port แนวคิดจากบอท US bot/status.py)
...
| ทุก 30 นาที (รอบแรกวินาที 600) | `catchup_job`: เรียก `startup_*_job` ของรายงานรายวันซ้ำ (heartbeat / digest / confirm / reminder / openpos / calendar / scan) — เก็บรอบที่พลาดเพราะเครื่องหลับ และรอบที่ล้ม · ข้าม job ที่ `_done` วันนี้แล้วและไม่เคยล้ม (ไม่ build ซ้ำเปล่า ๆ) · ไม่รวมเก็บตกข่าว · log INFO `catch-up: retrying failed job <name>` เฉพาะ job ที่เคยล้มวันนี้ (`key in _fail_counts`) — job ที่แค่ยังไม่ถึงเวลา (เช่น heartbeat ทุกรอบ, scan/openpos ก่อน 17:30/17:45) เงียบ กันสแปม ~55 บรรทัด/วัน |

- **job รายวันล้มต้องไม่เงียบ** (`_run_guarded`, port แนวคิดจาก HK/US): `X_job` / `startup_X_job` ทั้ง 12 ตัวเรียก core ผ่านตัวห่อนี้ — exception จากขั้น build หลุดออกมา (ไม่กลืนเงียบแล้ว) → นับใน `_fail_counts[(job, วัน)]` + broadcast "⚠️ <job> ล้มเหลว: … จะลองใหม่ ~30 นาที (n/3)" → `catchup_job` ลองใหม่ · ครบ `JOB_MAX_TRIES = 3` → แจ้งว่าหยุดลองวันนี้ · key รายวันใน `digest_state` ยังเขียนเฉพาะเมื่อส่งถึงเหมือนเดิม (= claim) · **ส่งล้มทุก chat ก็นับว่าล้ม** (ทุกรายงาน: มีของจะส่งแต่ไม่ถึงสักแชท = raise ให้ catch-up ลองใหม่ — ได้บางแชท = log พอ ไม่ส่งซ้ำ) · reminder/openpos build ต่อ chat: build ล้มทุก chat และไม่มีใครได้ก็นับว่าล้มเช่นกัน · นับใน memory — restart เริ่มใหม่
- **ดึงข่าว SET ล้ม: รู้สาเหตุ ล้มเร็ว เว้นระยะ แล้วไม่ลืมแจ้ง ✅** (incident 1 ก.ย. 2026 — ล้ม 16 รอบติด 17:05-19:35 รอบละ 90 วิ ทุกอาการหน้าตาเหมือนกันใน log แล้วรีบูตเช้าถัดมา รอบที่สำเร็จเงียบสนิท) · `set_news`: ดัก `page.on("response")` ก่อน `goto` — API ตอบไม่ใช่ 200 → `SetNewsBlocked(status)`, ได้หน้ากันบอท (`_is_challenge_html`) → `SetNewsChallenged` โยนทันทีที่ `goto` จบ ไม่รอ `expect_response` จนหมด `RESPONSE_TIMEOUT_S` (45 วิ — เป็นค่า default ของ `goto` ด้วย ไม่งั้นมี retry แล้วค้างได้ 3 นาที) · แต่ SPA มักยิง XHR ข่าว **หลัง** domcontentloaded (สรุปจาก incident 1 ก.ย. — หลักฐานที่มีคือ "ไม่มี 200 มาก่อนหมดเวลา") ใบที่ถูกปฏิเสธจึงมักมาตอนกำลังรอ → `_timeout_reason` แปลง timeout เป็น `SetNewsBlocked` ถ้าระหว่างรอมีใบ != 200 (ไม่งั้น log บอก 403 แต่ธง/สถานะบอก Timeout) · `_Incapsula_Resource` **ไม่ใช่** สัญญาณบล็อก (Imperva แทรกในหน้าปกติด้วย — นับเป็นบล็อกเมื่อไหร่ข่าวหยุดทั้งระบบ) (ยืนยันจาก log จริง 3 ก.ย. 2026 ว่าหน้าปกติมีสคริปต์นี้ — ไม่ log แล้ว) · ลองใหม่ `FETCH_RETRIES` (1) ครั้งด้วย context ใหม่เฉพาะสามอาการที่ "เปิดใหม่แล้วอาจผ่าน" (Blocked / Challenged / Timeout) อาการอื่นโยนออกทันที
- ล้มติดกัน ≥3 รอบ (~30 นาที) → ⚠️ เตือนครั้งเดียวต่อการล่มหนึ่งช่วง + เว้นระยะ poll ตาม `_news_backoff_minutes` (3-4 รอบ = 20 น. / 5-6 = 40 / ≥7 = 60 เก็บใน `_news_next_try` — ไม่เคยหยุดถาวร และ `startup_catchup_job` ไม่ติด backoff) · ธง `news_outage` (`since`/`fails`/`reason`) ลง `digest_state.json` ด้วย → ✅ "กลับมาปกติ" รอดข้ามรีสตาร์ท (เดิมตัวนับอยู่ใน memory ล้วน) ล้มซ้ำระหว่างล่มแค่อัปเดตธง ไม่ ⚠️ ซ้ำ และรับ `fails` จากธงมานับต่อ (ไม่งั้นรีสตาร์ทแล้ว backoff หล่นกลับเป็น 0) · ตอนหาย: ล้างธงลงดิสก์ให้จบ **ก่อน** `await` ส่งข้อความ (ถือ `digest_state` ข้าม await = กลืน key ของ job อื่น) และ `_news_recovery_sent` กัน ✅ ซ้ำถ้าเขียนดิสก์ไม่ผ่าน
- คำสั่ง `สถานะ` ต่อท้ายบรรทัดข่าว SET ว่า `· ล่าสุด: <อาการ> @HH:MM` (เมื่อล้ม ≥1 รอบ) และ `· backoff ถึง HH:MM` (ระหว่างเว้นระยะ) — ค่ามาจาก `_news_last_error`/`_news_next_try` ฉีดผ่าน `build_status_report` เหมือนค่า memory ตัวอื่น · รีสตาร์ทกลางช่วงล่ม (ตัวนับ `fails` ใน memory เริ่มที่ 0) ยังต่อท้าย `· ⚠️ ธงล่มค้างตั้งแต่ HH:MM (ล้ม n รอบ: เหตุผล) — รอ poll ถัดไปยืนยัน` ได้จากธง `news_outage` ที่ดิสก์ (เมื่อ `fails > 0` ไม่ต่อซ้ำ — memory สะท้อนอยู่แล้ว)
- คำสั่งผู้ใช้เป็น**ข้อความภาษาไทยธรรมดา** (สแกน / ติดตาม XXX / ลิสต์ / สถิติ / สถานะ / เงา / งบ XXX วันที่ / ปฏิทิน / ข่าวงบ / สรุปงบ N / ยืนยัน / พอร์ต N / ไม้ XXX / ซื้อ-ขาย XXX ราคา / เทรด / วิเคราะห์ XXX / คำอธิบายงบ XXX) ผ่าน `handle_text()` → `_dispatch_text()` และทุกคำสั่งมี alias อังกฤษ (scan/watch/unwatch/list/stats/status/shadow/earn/calendar/news/digest/confirm/port/size/buy/sell/trades/ta/mda) เช็คแบบ case-insensitive
...
- **`สถานะ` / `/status`** (`bot_status.py`, port แนวคิดจากบอท US `bot/status.py`): PID/uptime · commit ที่รันอยู่ (จด `.git/HEAD` ตอน import) เทียบ HEAD ปัจจุบัน → "⚠️ HEAD ใหม่กว่า — restart" · job ทุก slot วันนี้จาก `digest_state` + `_done`/`_fail_counts` ใน memory (✅ ส่งแล้ว / ✅ ทำแล้วไม่มีอะไรส่ง / ⚠️ ล้ม n/3 / ⛔ หยุดลอง / ❌ ถึงเวลายังไม่รัน / ⏳ / — วันหยุด — heartbeat/เตือนวันงบ หยุดแค่เสาร์-อาทิตย์ ที่เหลือหยุดวันหยุด SET ด้วย ตาม core จริง) · ลิสต์ของ chat นั้น · อายุข่าว SET + ล้มติดกัน · สแกนล่าสุดจาก `scan_log.csv` · WARNING/ERROR วันนี้ใน `bot_log.txt` — **ไม่ยิง API** อ่านไฟล์/memory ล้วน ตอบทันที · โมดูล**ไม่ import `telegram_bot`** (กัน import วน) ค่าใน memory ฉีดผ่าน `build_status_report` ใน telegram_bot · log ของ apscheduler ตั้งเป็น WARNING (เดิม 97% ของ `bot_log.txt` คือ "Running job…" — ไฟล์ 1 MB ×3 เก็บได้นานขึ้น ~40 เท่า)
...
- ไม้เงา (`เงา` / shadow.py): ทุกแถวใน `scan_log.csv` = ไม้จำลอง (ครั้งแรกต่อ ticker+วันงบ) เข้า open วันถัดจากวันสแกน / ตัดปิดหลุด `pre_earn_low` / ครบ 20 วันทำการ — replay จากราคาย้อนหลังตามปรัชญา "คำนวณย้อนหลังได้เสมอ" **ด้วยราคาจริงไม่ adjust** (`fetch_history(..., auto_adjust=False)` — สเกลเดียวกับเส้น ⛔ ที่จดไว้ ณ วันสแกน หุ้น SET ขึ้น XD หลังงบบ่อย; stats/backtest ยังใช้ adjusted ซึ่งถูกสำหรับวัด drift) ไม่มี job เฝ้า ไม้ปิดแล้ว cache ลง `shadow_log.csv` เรียกซ้ำยิง Yahoo เฉพาะไม้ที่ยังเปิด — BANDS ใช้ของ stats (lazy import ใน `_bands()` กัน import วนกับ stats ที่เรียก shadow)
...
- ปุ่มใต้ผลสแกน / สรุปงบ / ยืนยัน (`_watch_buttons`, เรียก 6 จุด): **หุ้นละแถว** `[➕ SYM watch:SYM] [📐 SYM ta:SYM] [📅 SYM url Earnings Radar]` — เพิ่มปุ่มต่อหุ้นให้แก้ที่ `_stock_row` จุดเดียว · ปุ่ม ➕ กับคำสั่ง "ติดตาม" ใช้ validate กลางตัวเดียวกัน (`_validated_watch_add`: regex + ลองดึงข้อมูลจริง — no-data ปฏิเสธ, Yahoo ล่มรับไว้ก่อน)
...
- `check_new_earnings_news()` นอกจากบันทึกวันงบ/ตัวเลข F45 แล้ว ยังต่อท้าย `filings_log.csv` (ทุกข่าวงบที่ยังไม่เคยเห็น — ดิบกว่า `earnings_results.csv` เพราะเก็บแม้ตัวที่อ่านตัวเลขไม่ได้) ใช้เป็นแหล่งข้อมูลของ "สรุปงบเช้า" · **mark seen เฉพาะเมื่อเขียน `filings_log` สำเร็จ** — เขียนล้ม (เช่น Excel เปิดไฟล์ค้าง) = log.warning + ข้ามแจ้งรอบนี้ ให้รอบถัดไป retry ทั้งข่าว (แจ้งช้าแทนแถวหายถาวร) และตัวเลขที่เขียน `earnings_results` ล้มจะคงอยู่ในคิว `f45_backlog` ลองเขียนใหม่รอบหน้า (ไม่นับ tries)
...
`.env` (BOT_TOKEN, DASHBOARD_TH_CACHE, DASHBOARD_SITE_DIR, DASHBOARD_URL, GEM_TA_URL) — อ่านผ่าน python-dotenv (`utf-8-sig`) — .env มี BOM ได้, `watchlist.json`, `chat_ids.json`, `news_seen.json`, `last_alive.json`, `watch_state.json`, `scan_log.csv`, `lookup_log.csv`, `backtest_results.csv`, `bot_log.txt`, `.yf_cache/`, `filings_log.csv` (ใครแจ้งงบเมื่อไหร่ — ดิบกว่า earnings_results.csv รวมตัวที่อ่านตัวเลขไม่ได้ด้วย), `digest_state.json` (กันส่งรายงานอัตโนมัติซ้ำ — key รายวัน `last_sent` / `confirm_last_sent` / `remind_last_sent` / `openpos_last_sent` / `alive_last_sent` / `scan_last_run` + รายสัปดาห์ `calendar_last_sent` เก็บ anchor วันอาทิตย์ล่าสุด + ธง `news_outage` ตอนดึงข่าว SET ล่ม), `port_settings.json` (ขนาดพอร์ตต่อ chat สำหรับคำนวณขนาดไม้), `trades_log.csv` (บันทึกไม้จริง — append-only เหมือน scan_log), `shadow_log.csv` (ไม้เงาที่ปิดแล้ว — cache สร้างใหม่ได้เสมอจาก scan_log), `f45_backlog.json` (คิว F45 ที่ยังไม่ได้อ่านตัวเลข — วันพีคเกินโควตาต่อรอบ)
