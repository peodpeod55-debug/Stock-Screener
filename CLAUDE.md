...
- **job รายวันล้มต้องไม่เงียบ** (`_run_guarded`, port แนวคิดจาก HK/US): `X_job` / `startup_X_job` ทั้ง 12 ตัวเรียก core ผ่านตัวห่อนี้ — exception จากขั้น build หลุดออกมา (ไม่กลืนเงียบแล้ว) → นับใน `_fail_counts[(job, วัน)]` + broadcast "⚠️ <job> ล้มเหลว: … จะลองใหม่ ~30 นาที (n/3)" → `catchup_job` ลองใหม่ · ครบ `JOB_MAX_TRIES = 3` → แจ้งว่าหยุดลองวันนี้ · key รายวันใน `digest_state` ยังเขียนเฉพาะเมื่อส่งถึงเหมือนเดิม (= claim) · reminder/openpos build ต่อ chat: ล้มทุก chat และไม่มีใครได้ค่อยนับว่าล้ม · นับใน memory — restart เริ่มใหม่
